...
AR=$(ls -d $MD/old/toma-larga-aberracion-* | head -1)
...
for b in p01 p02 p03 p04 p05 p06; do
  ...
  ffmpeg -nostdin -y -v error -i "$AR/$b.avi" -vf "$F" \
    -c:v libx264 -preset slow -crf 15 -pix_fmt yuv420p -c:a aac -b:a 192k "$T/$b.mp4" || exit 1
  echo "file '$T/$b.mp4'" >> "$T/lista.txt"
done
ffmpeg -nostdin -y -v error -f concat -safe 0 -i "$T/lista.txt" -c copy "$T/base.mp4"
# audio: el de ITER3, que dio 4.85 dB
IN="$T/base.mp4"; DUR=$(ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 "$IN")
ffmpeg -nostdin -y -v error -i "$IN" -vn \
...
ffmpeg -nostdin -y -v error -i "$T/voz.wav" -i "$T/lecho.wav" \
  -filter_complex "[0:a][1:a]amix=inputs=2:duration=first:normalize=0,loudnorm=I=-17:TP=-1.5:LRA=3" -c:a pcm_s16le "$T/mix.wav"
ffmpeg -nostdin -y -v error -i "$IN" -i "$T/mix.wav" -map 0:v -map 1:a -c:v copy -c:a aac -b:a 192k "$1"
