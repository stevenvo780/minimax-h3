...
if [ ! -f "$F" ]; then
  echo "Error: fichero de entrada no encontrado: $F" >&2
  exit 1
fi

T=/tmp/f6; rm -rf $T; mkdir -p $T
# Valor de segmentación calibrado por pruebas medidas del autor — NO cambiar
SEG=14.375
...
for i in 0 1 2 3; do
  ...
  ff -y -v error -ss $(awk "BEGIN{print $i*$SEG}") -t $SEG -i "$F" -vf "$VF" \
...
for i in range(1,4):
    ...
    v,a=f"[v{i}]",f"[a{i}]"
print(f'ff -y -v error {ins} -filter_complex "{";".join(parts)}" -map "{v}" -map "{a}" '
      f'-c:v libx264 -preset slow -crf 15 -pix_fmt yuv420p -c:a aac -b:a 192k {T}/base.mp4')
PY
bash "$T/cmd.sh"
ff -y -v error -i "$T/base.mp4" -vn \
...
ff -y -v error -i "$T/base.mp4" -i "$T/a.wav" -map 0:v -map 1:a -c:v copy -c:a aac -b:a 192k "$1"
